...
def average_weights(weights_list):
    """Perform federated averaging (simple average) of weights from all hospitals"""
    if not weights_list:
        return None
    ...
    for w in weights_list:
        coef = w["coef"][0]  # Logistic regression has single coefficient array
        intercept = w["intercept"][0]
...
def map_form_inputs_to_features(disease_folder, form_inputs):
    ...
    if disease_name == "asthma":
        ...
        # Map symptoms (binary inputs from checkboxes)
        symptom_map = {
            "tiredness": "Tiredness",
            "dry_cough": "Dry-Cough",
        ...
        if form_inputs.get("none_sympton") in ["1", "yes", "true"]:
            feature_dict["None_Sympton"] = 1
        if form_inputs.get("none_experiencing") in ["1", "yes", "true"]:
        ...
        # Gender (numeric: 0 or 1, typically 0=Male, 1=Female)
        gender = form_inputs.get("gender", "").lower()
    ...
    # Debug: Print feature vector for troubleshooting (can be removed later)
    # print(f"DEBUG: Feature vector length: {len(feature_vector)}, Features length: {len(features)}")
    # print(f"DEBUG: Feature dict keys: {list(feature_dict.keys())[:5]}...")
    # print(f"DEBUG: First few feature values: {feature_vector[:5]}")
...
def predict_with_averaged_model(disease_folder, feature_vector, use_fedavg_simulator: bool = True):
    """
    Make prediction using federated averaged weights - matches sklearn LogisticRegression.predict_proba
    
    Args:
        disease_folder: Disease folder name
        feature_vector: Feature vector for prediction
        use_fedavg_simulator: If True, use FedAvg simulator (simulates cloud aggregation)
                            If False, use simple local averaging
    """
    if use_fedavg_simulator and FedAvgSimulator is not None:
        # Use federated averaging simulator (simulates cloud-based aggregation)
        try:
        ...
        # Use simple local averaging (fallback)
        weights_list = load_all_hospital_weights(disease_folder)
        if not weights_list:
            return None
        avg_weights = average_weights(weights_list)
    ...
    classes = avg_weights["classes"]  # Should be [0, 1] for binary classification
    
    # Calculate decision function: z = w^T * x + b
    # This is equivalent to sklearn's decision_function
    z = sum(c * f for c, f in zip(coef, feature_vector)) + intercept
    
    # Sigmoid to get probability of class 1: P(y=1|x) = 1 / (1 + exp(-z))
    # This matches sklearn's LogisticRegression.predict_proba for binary classification
    import math
    try:
        # Prevent overflow
        if z > 700:
    ...
    # Probability of class 0 (negative)
    probability_negative = 1.0 - probability_positive
    
    # Class prediction (threshold 0.5, same as sklearn's predict method)
    if probability_positive >= 0.5:
        prediction_class = 1  # Positive
        predicted_label = "Positive"
        # Confidence is the probability of the predicted class
        confidence_prob = probability_positive
    else:
        prediction_class = 0  # Negative
    ...
    return {
        ...
        "probability": probability_positive,  # Probability of positive class
        "confidence": confidence_prob,  # Confidence in the prediction (probability of predicted class)
        "prob_negative": probability_negative,  # For reference
...
Please provide helpful health tips and guidance based on the user's question. Keep responses concise (use simple bullet points to answer) and always remind users to consult their healthcare provider for medical concerns.
